...
const ServiceSingle = ({service}: Props) => {
	...
	return (
		<div key={service.id} className="mb-12 bg-gray-100 dark:bg-zinc-900 rounded-2xl p-12 relative overflow-hidden">
			{service.id % 2 === 1 ? (
				<>
			...
			<div className="max-w-4xl mx-auto z-20 relative">
				{service.icon()}
				<h2 className="text-4xl text-black dark:text-white font-extrabold text-center uppercase">{service.name}</h2>
				<p className="text-lg mt-5 text-gray-700 dark:text-gray-300 text-center">{service.description}</p>
			</div>
			<div className="flex flex-col mt-8 space-y-4 sm:flex-row sm:justify-center sm:space-y-0 sm:space-x-6">
				<Button
					...
					variant="flat"
					size="md"
					className={`bg-${service.mainColor}-200 text-base font-medium text-${service.mainColor}-700 dark:bg-${service.mainColor}-900 dark:text-${service.mainColor}-300`}
				>
					Подробнее
