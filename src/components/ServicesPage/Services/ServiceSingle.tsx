...
const ServiceSingle = ({service}: Props) => {
	...
	return (
		<div key={service.id} className="mb-12 bg-gray-100 dark:bg-zinc-900 rounded-2xl p-6 md:p-12 relative overflow-hidden">
			{service.id % 2 === 1 ? (
				<>
					<span className={`absolute w-48 h-48 right-0 rounded-full bottom-0 bg-${service.mainColor}-700 blur-3xl translate-x-16 translate-y-16 opacity-80`}></span>
					<span className={`absolute w-48 h-48 left-0 rounded-full top-0 bg-${service.mainColor}-700 blur-3xl -translate-x-16 -translate-y-16 opacity-80`}></span>
				</>
			) : (
				<>
					<span className={`absolute w-48 h-48 right-0 rounded-full top-0 bg-${service.mainColor}-700 blur-3xl translate-x-16 -translate-y-16 opacity-80`}></span>
					<span className={`absolute w-48 h-48 left-0 rounded-full bottom-0 bg-${service.mainColor}-700 blur-3xl -translate-x-16 translate-y-16 opacity-80`}></span>
				</>
			)}
			...
			<div className="flex flex-col mt-8 space-y-4 sm:flex-row sm:justify-center sm:space-y-0 sm:space-x-6">
				<Button
					href={`/services${service.detailLink}`}
					as={Link}
					color="default"
					variant="flat"
					size="md"
