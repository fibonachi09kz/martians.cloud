...
const ThemeSwitcher = () => {
	...
	return (
		<div className="flex justify-center border rounded-full border-gray-300 dark:border-white/20 gap-2 p-1 w-fit">
			<button
				className={`p-1 rounded-full ${isActive("light") ? "bg-gray-200 dark:bg-white/20 text-white" : ""}`}
				onClick={() => setTheme('light')}
			>
				<SunIcon className="w-4 h-4 dark:text-white text-gray-600" />
			</button>
			<button
				className={`p-1 rounded-full ${isActive("dark") ? "bg-gray-200 dark:bg-white/20 text-white" : ""}`}
				onClick={() => setTheme('dark')}
			>
				<MoonIcon className="w-4 h-4 dark:text-white text-gray-600" />
			</button>
			<button
				className={`p-1 rounded-full ${isActive("system") ? "bg-gray-200 dark:bg-white/20 text-white" : ""}`}
