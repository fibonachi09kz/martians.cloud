...
const ThemeSwitcher = () => {
	...
	if (!mounted) {
		return (
			<div className="flex justify-center items-center border rounded-full border-gray-300 dark:border-gray-800 gap-2 p-1 w-[96px] h-[34px]">
	...
	return (
		<div className="flex justify-center border rounded-full border-gray-300 dark:border-gray-800 gap-2 p-1 w-fit">
			<button
				className={`p-1 rounded-full ${isActive("light") ? "bg-gray-200 dark:bg-gray-800 text-white" : ""}`}
				onClick={() => setTheme('light')}
			>
				<SunIcon className="w-4 h-4 dark:text-white text-gray-600" />
			</button>
			<button
				className={`p-1 rounded-full ${isActive("dark") ? "bg-gray-200 dark:bg-gray-800 text-white" : ""}`}
				onClick={() => setTheme('dark')}
			>
				<MoonIcon className="w-4 h-4 dark:text-white text-gray-600" />
			</button>
			<button
				className={`p-1 rounded-full ${isActive("system") ? "bg-gray-200 dark:bg-gray-800 text-white" : ""}`}
